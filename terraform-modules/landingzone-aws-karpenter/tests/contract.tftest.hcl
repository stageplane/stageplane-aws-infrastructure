...
# Intent
# -----------------------------------------------------------------------------
# Verifies the landing-zone Karpenter composition contract by overriding the
# child module outputs while keeping the test cloud-independent.
# -----------------------------------------------------------------------------
...
run "exports_landingzone_karpenter_contract" {
  command = plan
  ...
  assert {
    ...
    error_message = "The landing-zone Karpenter module must expose its default disabled state."
  }

  assert {
    condition     = output.namespace == "karpenter"
    error_message = "The landing-zone Karpenter module must expose the controller namespace when enabled."
  }
}
