...
variable "cluster_name" {
  description = "Canonical cluster name."
  type        = string
}

variable "cluster_version" {
  description = <<-EOT
  Kubernetes minor version for the cluster.
...
variable "vpc_id" {
  description = "VPC identifier used by the EKS control plane and worker networking."
  type        = string
}

variable "private_subnet_ids" {
  description = "Private subnet identifiers used by the control plane and node groups."
  type        = list(string)
}
...
variable "cluster_endpoint_public_access_cidrs" {
  description = <<-EOT
  Source CIDR allow-list for the public EKS API endpoint. This module passes
  the value directly to the upstream EKS module and intentionally avoids the
  upstream open-to-world default.
  EOT
  type        = list(string)
...
variable "common_tags" {
  description = "Repository-standard tags applied to cluster resources."
  type        = map(string)
  default     = {}
}
